...
def parse_vscmds(path, rx, ti):
    ...
    for pkt in pkts:
        ...
        if pkt_datetime < ti[0] or pkt_datetime > ti[1]:
            ...
            # NOTE: remove first 14 bytes and last 4 butes
...
if __name__ == "__main__":
    ...
    # NOTE: export log with datetime +2 hours from our timezone
    start_datetime = datetime(2020, 6,  11,  13+2, 24, 00, 0)
    end_datetime =   datetime(2020, 6,  11,  16+2, 34, 00, 0)
    ...
    start_datetime = datetime(2020, 6,  11,  13+2, 24, 00, 0)
    end_datetime =   datetime(2020, 6,  11,  13+2, 54, 00, 0)
    ti = (start_datetime, end_datetime)
    for i in [23, 24]:
        results = parse_hcievents(path, i, ti)
        put_results('scenario01-23-24', results)

    create_db('scenario02-23-18', rm_old=True)
    start_datetime = datetime(2020, 6,  11,  13+2, 56, 00, 0)
    end_datetime =   datetime(2020, 6,  11,  14+2, 26, 00, 0)
    ti = (start_datetime, end_datetime)
    for i in [23, 18]:
        results = parse_hcievents(path, i, ti)
        put_results('scenario02-23-18', results)

    create_db('scenario03-23-10', rm_old=True)
    start_datetime = datetime(2020, 6,  11,  14+2, 28, 00, 0)
    end_datetime =   datetime(2020, 6,  11,  14+2, 58, 00, 0)
    ti = (start_datetime, end_datetime)
    for i in [23, 10]:
        results = parse_hcievents(path, i, ti)
        put_results('scenario03-23-10', results)


    create_db('scenario04-23-08', rm_old=True)
    start_datetime = datetime(2020, 6,  11,  15+2, 00, 00, 0)
    end_datetime =   datetime(2020, 6,  11,  15+2, 30, 00, 0)
    ti = (start_datetime, end_datetime)
    for i in [23,  8]:
        results = parse_hcievents(path, i, ti)
        put_results('scenario04-23-08', results)


    create_db('scenario05-23-06', rm_old=True)
    start_datetime = datetime(2020, 6,  11,  15+2, 32, 00, 0)
    end_datetime =   datetime(2020, 6,  11,  16+2,  2, 00, 0)
    ti = (start_datetime, end_datetime)
    for i in [23,  6]:
        results = parse_hcievents(path, i, ti)
        put_results('scenario05-23-06', results)

    create_db('scenario06-23-02', rm_old=True)
    start_datetime = datetime(2020, 6,  11,  16+2,  4, 00, 0)
    end_datetime =   datetime(2020, 6,  11,  16+2, 34, 00, 0)
    ti = (start_datetime, end_datetime)
    for i in [23,  2]:
        results = parse_hcievents(path, i, ti)
        put_results('scenario06-23-02', results)
